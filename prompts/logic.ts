...
- 时间与地点（仅从 [当前世界时间] 读取）：
  ...
  - gameState.当前地点 / gameState.世界坐标 / gameState.当前区域 / gameState.天气
...
- 变量状态快照（强制扫描，仅以此为事实基础）：
  - 生理扫描：
    ...
    - gameState.角色.状态 / 异常 / 疲劳度
    - gameState.角色.程序槽位 (Program Slots)
  - 库存审计：
    - gameState.角色.装备（主手/副手/头部/身体/手部/腿部/足部/饰品）
    - gameState.背包（InventoryItem 完整结构）
    - gameState.角色.发展能力 / 技能 / 战术程序（可用清单）
  - 战利品与公共战利品：
    - gameState.公共战利品 / gameState.战利品 / gameState.战利品背负者 / gameState.势力.仓库
  - 情报扫描：
    - gameState.任务 / gameState.剧情 / gameState.契约
    - gameState.世界（头条新闻/街头传闻/势力格局/区域战争等）
...
- 必须检查：
  ...
  - 经验值与黑匣子数据：仅在战斗结算明确发生时 add gameState.角色.经验值 / gameState.角色.黑匣子数据；禁止在 logs 明示数值或“结算”措辞。
  - 掉落与拾取：
    - 仅叙事写“掉落/散落”但未拾取 → 不生成指令。
    - 明确“拾取/装入公共包/交给背负者” → push gameState.公共战利品。
    - 明确“分配/归档/入库” → push gameState.战利品 或 gameState.势力.仓库。
  - 配给券与物资收益：仅在叙事明确发生时 add gameState.角色.配给券。
